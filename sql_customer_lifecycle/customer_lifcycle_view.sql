/*
schema name : public
table names : policy ,calendar,finance

to change the schema edit the query schema_policy and schema_calendar

*/ WITH schema_policy AS
  (SELECT *
   FROM public.policy
...
*/
  ...
  (SELECT b.user_id,
          a.year_month,
          'Active' AS lifecycle
   FROM schema_calendar AS a
   LEFT JOIN schema_policy AS b ON a.date BETWEEN b.policy_start_date AND b.policy_end_date
   GROUP BY b.user_id ,
            a.year_month
   ORDER BY b.user_id),
        /*
